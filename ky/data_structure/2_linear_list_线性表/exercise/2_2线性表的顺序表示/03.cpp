...
/*
    ...
    输出:
        void
    分析思路:
        从头开始遍历顺序表，并与 x 进行比对，如果比对上，则删除掉该值
    实现细节:
        删除 x 后需要把后续的元素向前移动，假设顺序表元素个数为 n，值为 x 的元素个数为 m，
        则剩下的 n-m 个元素分别放到位序 [1, n-m]的位置，且相对位置不变
            定义变量 after_pos 表示删除所有 x 后移动完成后的位置，当遍历到的值不等于 x 则进行加1,当遍历到的值等于x，则不变
            定义变量 before_pos 表示未删除 x 前的所有元素位置，每次循环都会变化
    注意点:
...
void DeleteX(SqList<int>& L, int x)
{
    //1. 判空
    if(L.Empty())
    {
