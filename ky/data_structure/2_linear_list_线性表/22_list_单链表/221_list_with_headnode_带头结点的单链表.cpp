...
LNode* GetElem(const LinkList& L, int i)
{
    ...
    //需要注意
    //  3.1 当位序 i 大于链表长度时，返回 nullptr
    //  3.2 带头结点的链表需要从第一个非头结点的结点开始遍历计数
    LNode* tmp = L;
    int pos = 0;
    while(tmp && pos < i)
    {
    ...
    if(tmp == nullptr)
    {
        cout << "当前链表长度为:" << Length(L) << " ,但需要查找链表中第" << i << "个位置的结点，查找失败" << endl;
    }
    else
    {
...
//5. 在带头结点单链表 L 中的第 i 个位置上插入指定元素 e
/*
...
bool ListInsert(LinkList &L, int i, int e)
{
    ...
    if(tmp == nullptr)
    {
        cout << "当前链表长度为:" << Length(L) << " ,但需要在链表中第" << i << "个位置插入 " << e << " ,插入失败" << endl;
        return false;
    }
...
/*
    功能: 删除单链表 L 指定位序 i 处的结点,并把其值用 e 返回
    输入:
        LinkList &L: 待删除结点的链表
        int i: 位序
        int e: 传值
...
bool ListDelete(LinkList &L, int i, int e)
{
    ...
    {
        cout << "当前链表长度为:" << Length(L) << " ,但需要在链表中第" << i << "个位置删除结点" << ",删除失败" << endl;
        return false;
    }
    //修改指针域并删除结点
    LNode* mid = tmp->next;
...
int main()
{
    ...
    InsertNextNode(L, 66);
    InsertNextNode(L, 88);
    InsertPriorNode_w1(L, L->next, 77);
    InsertPriorNode_w1(L, L->next->next, 99);
    InsertPriorNode_w2(L, L->next, 1024);
    InsertPriorNode_w2(L, L, 2048);
    PrintList(L);
    cout << "表是否为空:" << (Empty(L)?"是":"否") << endl;
    cout << "表长:" << Length(L) << endl;

    //测试按位序删除结点操作
    cout << "*******************测试按位序删除结点操作*****************" << endl;
    int result = 0;
    ListDelete(L, 3, result);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;

    ListDelete(L, 10, result);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;

    ListDelete(L, 1, result);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;
    ...
    cout << "*******************测试根据指针删除结点操作*****************" << endl;
    DeleteNode_w1(L, L->next);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;

    DeleteNode_w1(L, L->next->next);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;

    DeleteNode_w2(L, L->next);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;

    DeleteNode_w2(L, L->next->next->next);
    PrintList(L);
    cout << "表长:" << Length(L) << endl;

    cout << "*******************测试根据位序查找结点操作*****************" << endl;
    LNode* val = GetElem(L, 2);
    if(val)
    {
        cout << "值为:" << val->data << endl;
    }

    cout << "*******************测试按值查找结点操作*****************" << endl;
    val = LocateElem(L, 3);
